...
export function createClipPages(env: { assetLinksSha256?: string }) {
  ...
  pages.get('/c/:id', async (c) => {
    ...
    if (clip.deleted_at !== null) {
      return c.html(page(episode.title, `<h1>This clip was removed</h1><h2>${esc(episode.title)}</h2>${show}<p class="muted">The episode is still there.</p><a class="btn" href="${open}">Open the episode in the app</a><p class="muted">No app yet? Get SocialMorning for Android.</p>`));
    }
    ...
    return c.html(page(episode.title, `<h1>${esc(episode.title)}</h1>${show}${clip.caption ? `<blockquote>${esc(clip.caption)}</blockquote>` : ''}${by}<a class="btn" href="${open}">Open in app</a><p class="muted">No app yet? Get SocialMorning for Android — the clip plays from the publisher's own audio; nothing is hosted here.</p>`));
  });
