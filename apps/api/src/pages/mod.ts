...
async function ownerFromCookie(c: Context<AuthEnv>): Promise<{ owner: Listener; token: string } | undefined> {
  ...
  if (!l) return undefined; // G9 BROKEN ON PURPOSE: any signed-in cookie is the owner
...
mod.post('/login', async (c) => {
  ...
  if (!row || !ownerId || !(await verifyPassword(password, row.password_hash))) { // G9 BROKEN ON PURPOSE
