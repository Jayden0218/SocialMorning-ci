...
it('filterComments: blocked authors out, a blocked reply under a kept parent is a placeholder, a reported comment stays as "You reported this"; no sets → unchanged', () => {
  ...
  expect(out.map((x) => x.id)).toEqual(['t1', 't3']);
  expect(out[1]).toMatchObject({ id: 't3', reported: true, deleted: true, body: null });
...
it('filterClips / filterFeed / filterListeners', () => {
  ...
  const fc = filterClips([clip('k1', 'A'), clip('k2', 'B'), clip('k3', 'A')], { blocked: new Set(['B']), hidden: new Set([hiddenKey('clip', 'k3')]) });
  expect(fc.map((k) => k.id)).toEqual(['k1', 'k3']);
  expect(fc[1]).toMatchObject({ id: 'k3', reported: true, caption: '' });
